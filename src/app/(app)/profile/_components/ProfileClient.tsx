...
function calcCompletion(p: Profile): number {
  const fields = [
    p.firstName, p.lastName, p.displayName, p.username,
    p.primaryEmail, p.profilePhoto, p.phoneNumber,
    p.alternateEmail, p.country, p.cityTown,
    p.streetAddress, p.emergencyName, p.emergencyPhone,
    p.emergencyRelation,
  ];
...
export default function ProfileClient({ profile }: Props) {
  ...
  function handleCancel() {
    // Reset all fields back to original profile values
    setFirstName(profile.firstName);
    setLastName(profile.lastName);
    setOtherNames(profile.otherNames ?? "");
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {isIncomplete && !isEditing && (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 flex items-start gap-3">
          ...
          <div>
            <div className="text-sm font-semibold text-amber-800">Your profile is not complete</div>
            <div className="text-xs text-amber-600 mt-0.5">
              Please complete it to get the most out of MUNIX.{" "}
              <button
                onClick={() => setIsEditing(true)}
                className="underline font-medium hover:text-amber-800 transition"
              >
                Complete it now
              </button>
            </div>
      ...
      <div className="mb-6 rounded-xl border border-gray-100 bg-white p-6">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {!phoneNumber && <span className="text-xs bg-amber-50 text-amber-600 border border-amber-200 px-2 py-1 rounded-full">+ Add phone number</span>}
          {!profilePhoto && <span className="text-xs bg-amber-50 text-amber-600 border border-amber-200 px-2 py-1 rounded-full">+ Add profile photo</span>}
          {!country && <span className="text-xs bg-amber-50 text-amber-600 border border-amber-200 px-2 py-1 rounded-full">+ Add address</span>}
          {!emergencyName && <span className="text-xs bg-amber-50 text-amber-600 border border-amber-200 px-2 py-1 rounded-full">+ Add emergency contact</span>}
          {completion === 100 && <span className="text-xs bg-green-50 text-green-600 border border-green-200 px-2 py-1 rounded-full">✓ Profile complete!</span>}
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6">
            ...
            <div className="flex items-center gap-5 mb-6">
              <div className="h-20 w-20 rounded-full bg-indigo-100 flex items-center justify-center text-xl font-bold text-indigo-600 overflow-hidden flex-shrink-0">
                {profile.profilePhoto ? (
                  <img src={profile.profilePhoto} alt={profile.displayName} className="h-full w-full object-cover" />
                ) : initials}
              </div>
              <div>
                <div className="text-lg font-bold text-gray-900">{profile.displayName}</div>
                <div className="text-sm text-gray-500">@{profile.username}</div>
              </div>
            </div>
            <div className="mb-4 rounded-lg bg-indigo-50 border border-indigo-100 px-4 py-3 flex items-center justify-between">
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Primary Phone" value={profile.phoneNumber} />
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="State / Region" value={profile.stateRegion} />
              <Field label="City / Town" value={profile.cityTown} />
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Contact Name" value={profile.emergencyName} />
        ...
        <div className="space-y-6">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={[
                      ...
                      usernameStatus === "taken" ? "border-rose-300 focus:ring-rose-400/20"
                        : usernameStatus === "available" ? "border-green-300 focus:ring-green-400/20"
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-6">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <input value={profile.primaryEmail} disabled
          ...
          <div className="flex gap-3">
            <button
              ...
              className={[
                "flex-1 rounded-lg px-5 py-3 text-sm font-semibold text-white transition",
            ...
            <button
              ...
              className="flex-1 rounded-lg px-5 py-3 text-sm font-semibold text-gray-700 border border-gray-200 hover:bg-gray-50 transition"
            >
              Cancel
            </button>
          </div>
